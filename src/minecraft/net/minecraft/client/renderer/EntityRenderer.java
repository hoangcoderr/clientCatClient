...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    private void orientCamera(float partialTicks) {
        Entity entity = this.mc.getRenderViewEntity();
        float f = ModInstances.getSmoothSneaking().getEyeHeight(partialTicks);
        ...
        if (Reflector.EntityViewRenderEvent_CameraSetup_Constructor.exists()) {
            if (!this.mc.gameSettings.debugCamEnable) {
                float f6 = ModInstances.getPerspective().getCameraYaw() + (ModInstances.getPerspective().getCameraYaw() - ModInstances.getPerspective().getCameraYaw()) * partialTicks + 180.0F;
                float f7 = ModInstances.getPerspective().getCameraPitch() + (ModInstances.getPerspective().getCameraPitch() - ModInstances.getPerspective().getCameraPitch()) * partialTicks;
                float f8 = 0.0F;
    ...
    private void renderWorldDirections(float partialTicks) {
        if (this.mc.gameSettings.showDebugInfo && !this.mc.gameSettings.hideGUI && !this.mc.thePlayer.hasReducedDebug() && !this.mc.gameSettings.reducedDebugInfo) {
            ...
            GlStateManager.loadIdentity();
            this.orientCamera(partialTicks);
            GlStateManager.translate(0.0F, ModInstances.getSmoothSneaking().getEyeHeight(partialTicks), 0.0F);
